 ...
 *
 * **The scope is one fixed prefix.** The header exists for the OpenCode Go / Zen
 * routes, and membership is decided by the route key itself: every route that
 * *begins with* `opencode-go` is in scope — pi-ai's builtin `opencode-go`, this
 * deployment's `opencode-go-custom`, and any sibling route added later — and
 * nothing else is, under any configuration. There is deliberately no `routes:`
 * pattern, no opt-in list and no exclusion list: a configuration surface that
 * can widen this would be a surface that can send a gateway-specific header to a
 * provider that never asked for it. A route outside the prefix is left
 * byte-for-byte alone, and the status report proves where the line falls by
 * previewing the decision for representative routes and protocols.
 *
...
/**
 * The scope: every route key that begins with this prefix, matched
 * case-insensitively. `opencode-go` is pi-ai's builtin provider key for the
 * OpenCode Go / Zen gateway, and `opencode-go-custom` is the route this
 * deployment configured on top of it — but the rule is deliberately a prefix
 * rather than a list, so a new `opencode-go…` route is covered the moment it is
 * registered, with no configuration and no new release.
 */
export const SCOPED_ROUTE_PREFIX = 'opencode-go';

/**
 * The scope as a human reads it: shown in the status report, in the boot log,
 * and in the error a configuration attempt gets.
 */
export const SCOPED_ROUTE_PATTERN = `${SCOPED_ROUTE_PREFIX}*`;

/**
 * Representative routes the status preview evaluates, so the fixed line is
 * visible at a glance: two inside the prefix and two outside it.
 */
const PREVIEW_ROUTES = ['opencode-go', 'opencode-go-custom', 'opencode-zen', 'deepseek'];
  ...
  for (const key of Object.keys(raw)) {
    if (!CONFIG_KEYS.has(key)) {
      throw new SessionHeaderError(
        ...
        + `The route scope is fixed: every route beginning with ${SCOPED_ROUTE_PREFIX} (${SCOPED_ROUTE_PATTERN}) receives ${HEADER_NAME}, and no other route does.`,
      );
    }
  }
 ...
 *
 * A case-insensitive **prefix** test on the route key: `opencode-go`,
 * `opencode-go-custom` and `opencode-go-eu` are in; `opencode-zen`,
 * `my-opencode-go` and `opencode` are out. The rule is a literal prefix rather
 * than a separator-aware one, which is exactly the point: every route name this
 * gateway family uses begins with the gateway's own key.
 *
...
export function isScopedRoute(provider) {
  return typeof provider === 'string' && provider.toLowerCase().startsWith(SCOPED_ROUTE_PREFIX);
}
...
export function createSessionHeader(raw, internals = {}) {
  ...
  function hookRegistry(registry) {
    ...
    if (swept > 0 && ![...routesSeen].some((route) => isScopedRoute(route))) {
      diagnose('warn', `no route in the LLM registry begins with ${SCOPED_ROUTE_PREFIX}; the registry holds ${[...routesSeen].join(', ') || 'no routes'}, so the header stays off until such a route is registered`);
    }
    return swept;
  }
