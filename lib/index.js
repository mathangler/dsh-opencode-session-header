 ...
 */
import { createSessionHeader, HEADER_NAME, SCOPED_ROUTE_PATTERN } from './session-header.js';
...
function apply(ctx, config) {
  ...
  function sweep() {
    ...
    if (hooked > 0 && ctx.logger !== undefined && typeof ctx.logger.info === 'function') {
      ctx.logger.info(`opencode-session-header: hooked ${hooked} pi-ai adapter${hooked === 1 ? '' : 's'}; ${HEADER_NAME} is added per conversation on ${SCOPED_ROUTE_PATTERN} models only`);
    }
    return hooked;
  }
  ...
  if (!policy.enabled) {
    ...
    ctx.logger.info(`opencode-session-header: active (${HEADER_NAME} on ${SCOPED_ROUTE_PATTERN}, value mode ${policy.value})`);
  }
